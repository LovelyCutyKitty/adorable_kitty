...
(() => {
  ...
  const periodBounds = () => {
    ...
    if (range === 'week') {
      const start = new Date(current); start.setDate(current.getDate() - ((current.getDay() + 6) % 7));
      const end = new Date(start); end.setDate(start.getDate() + 6);
      return {from: isoDate(start), to: isoDate(end)};
    }
    if (range === 'all') return {from: '2026-07-30', to: '9999-01-01'};
  ...
  const activeShipments = () => {
    ...
    return all.filter(item => (!bounds.from || item.date >= bounds.from) && (!bounds.to || item.date <= bounds.to));
  };
  ...
  function renderShipping() {
    ...
    const bounds = periodBounds();
    view.innerHTML = `<input id="productSearch" hidden><section id="productContent" hidden></section><div class="page-head"><div><h2>출고 관리</h2><p>발주처별로 출고 대기 제품과 출고 기록을 확인합니다.</p></div></div><div class="subtabs"><button class="subtab ${mode === 'pending' ? 'active' : ''}" data-shipping-mode="pending">출고 대기</button><button class="subtab ${mode === 'history' ? 'active' : ''}" data-shipping-mode="history">출고 기록</button></div>${mode === 'history' ? `<div class="shipping-filter"><div class="shipping-range-tabs"><button type="button" data-shipping-range="week" class="${range === 'week' ? 'active' : ''}">이번 주</button><button type="button" data-shipping-range="month" class="${range === 'month' ? 'active' : ''}">이번 달</button><button type="button" data-shipping-range="all" class="${range === 'all' ? 'active' : ''}">전체 기간</button></div><div class="shipping-period"><span>조회 기간</span><input id="shippingFrom" aria-label="출고 기록 시작일" type="date" value="${esc(bounds.from)}"><b>~</b><input id="shippingTo" aria-label="출고 기록 종료일" type="date" value="${esc(bounds.to)}"></div></div>` : '<p class="edit-note">출고 가능 제품을 발주처별로 확인한 뒤 출고 처리하세요.</p>'}<section id="shippingContent" class="groups">${mode === 'pending' ? pendingHtml() : historyHtml()}</section>`;
  ...
  function init() {
    if (!document.querySelector('#shippingStyle')) document.head.insertAdjacentHTML('beforeend', `<style id="shippingStyle">
      .shipping-filter{display:grid;gap:10px;margin:14px 0 16px}.shipping-range-tabs{display:flex;gap:8px}.shipping-range-tabs button{flex:1;border:1px solid #c9dbd3;border-radius:12px;padding:10px 12px;background:#e8f0ec;color:#205446;font:inherit;font-weight:700;cursor:pointer}.shipping-range-tabs button.active{background:#12493f;border-color:#12493f;color:#fff}.shipping-period{display:flex;align-items:center;gap:8px;padding:10px 12px;background:#fff;border:1px solid #c9dbd3;border-radius:12px;color:#54746a;font-size:.9rem}.shipping-period span{white-space:nowrap;font-weight:700}.shipping-period b{color:#729087}.shipping-period input{min-width:0;flex:1;border:0;background:transparent;color:#183e35;font:inherit;outline:none}@media(max-width:480px){.shipping-period{flex-wrap:wrap}.shipping-period span{width:100%}.shipping-period input{min-width:120px}}
    </style>`);
