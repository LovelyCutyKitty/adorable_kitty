...
function productCard(p){const reserved=openOrdersFor(p), available=availability(p);return `<article class="item"><div class="item-top"><strong>${esc(p.name)}</strong><span class="badge ${available<0?'red':''}">${available<0?'${unit(Math.abs(available))} 부족':'여유 '+unit(available)}</span></div><p class="spec">${esc(p.spec)}</p><div class="item-meta"><span>현재 ${unit(p.stock)}${p.unit}</span><span>출고예정 ${unit(reserved)}${p.unit}</span></div><div class="item-actions"><button data-product="${p.id}" type="button">수정</button><button class="primary" data-production="${p.id}" type="button">생산 완료 입력</button></div></article>`;}
